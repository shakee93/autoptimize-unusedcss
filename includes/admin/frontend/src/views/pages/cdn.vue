            ...
            <div>
              <div>
                <button @click="purgeCach" :disabled="loading" class="disabled:opacity-50 flex mb-3 transition duration-300 bg-purple font-semibold text-white py-2 px-4 border border-gray-button-border hover:border-transparent rounded-lg">
                  <svg :class="loading? 'block' : 'hidden-important'" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <div class="grid mb-5">
            ...
            <div class="flex">
              ...
              <div class="mt-2.5 -ml-9 cursor-pointer z-50">
                <svg :class="{'animate-spin': refresh_element}" @click="validateCDN"
              ...
              <div class="ml-2">
                <button @click="copy" :disabled="loading"
                        ...
                        class="flex pt-2 -mr-[35px] transition duration-300 bg-transparent font-semibold text-black py-1.5 px-2 border border-transparent rounded-r-lg">
                ...
                <div :class="copied ? 'rl-Show' : 'rl-Hide'" class="-mt-20 px-2 -ml-[70px]" >
...
export default {
  ...
  methods:{
    ...
    doc(){
      window.open('https://docs.rapidload.io/features/cloud-delivery', '_blank');
    },
    ...
    purgeCach(){
      ...
      axios.post(window.uucss_global.ajax_url + '?action=purge_rapidload_cdn&nonce='+window.uucss_global.nonce , {
        headers: {
      ...
      axios.post(window.uucss_global.ajax_url + '?action=validate_cdn&dashboard_cdn_validator' , {
        headers: {
          ...
          .then(response => {
            ...
            this.uucss_cdn_dns_id = response.data.data.uucss_cdn_dns_id;
            this.uucss_cdn_url = response.data.data.uucss_cdn_url;
            this.uucss_cdn_zone_id = response.data.data.uucss_cdn_zone_id;
