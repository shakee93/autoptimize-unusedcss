    ...
    public function update_content($state){
        ...
        if(isset($this->options['delay_javascript']) && $this->options['delay_javascript'] == "1" || apply_filters('rapidload/delay-script/enable', false)){
           ...
           //!injected by RapidLoad \n!(function(){var supportedEvents=['DOMContentLoaded','readystatechanges','load'];var supportedTargets=[window,document];var originalDispatchEvent=EventTarget.prototype.dispatchEvent;var originalAddEventListener=EventTarget.prototype.addEventListener;var originalRemoveEventListener=EventTarget.prototype.removeEventListener;var capturedEvents=[];var dispatchedEvents=[];window.RaipdLoadJSDebug={capturedEvents:capturedEvents,dispatchedEvents:dispatchedEvents};EventTarget.prototype.addEventListener=function(type,listener,...options){var isDispatched=!type.includes('RapidLoad:')&&!!dispatchedEvents.find(e=>e.type===type)&&supportedTargets.includes(this);var catchEvent=!type.includes('RapidLoad:')&&!type.includes(':norapidload')&&supportedEvents.includes(type)&&supportedTargets.includes(this)&&typeof listener==='function';if(catchEvent||catchEvent!==isDispatched&&isDispatched){if(this===document||this===window){}if(this===document&&document.readyState!=='loading'||this===window&&document.readyState!=='loading'){setTimeout(()=>{listener.call(this,new Event(type))},30)}else{capturedEvents.push({target:this,type:type,listener:listener,options:options})}}if(type.includes(':norapidload')){type=type.replace(':norapidload','')}originalAddEventListener.call(this,type,listener,...options)};EventTarget.prototype.removeEventListener=function(type,listener,...options){if(supportedEvents.includes(type)&&supportedTargets.includes(this)){capturedEvents=capturedEvents.filter(e=>!(e.type===type&&e.listener===listener&&e.target===this))}originalRemoveEventListener.call(this,type,listener,...options)};EventTarget.prototype.dispatchEvent=function(event){dispatchedEvents.push(event);if(supportedEvents.includes(event.type)&&supportedTargets.includes(this)){capturedEvents=capturedEvents.filter(e=>{if(e.type===event.type&&e.target===this){e.target.removeEventListener(e.type,e.listener,...e.options);return false}return true})}return originalDispatchEvent.call(this,event)};supportedEvents.forEach(function(eventType){supportedTargets.forEach(function(target){target.addEventListener(eventType,function(){})})})})();";
            ...
            $content = "//!injected by RapidLoad \n!(function(){function mergeDuplicateObjects(array){const mergedObjects={};array.forEach(item=>{if(!mergedObjects[item.id]){mergedObjects[item.id]={...item}}else{Object.keys(item).forEach(key=>{if(item[key]!==null&&item[key]!==undefined){if(mergedObjects[item.id][key]===null||mergedObjects[item.id][key]===undefined){mergedObjects[item.id][key]=item[key]}}})}});return Object.values(mergedObjects)}var totalScripts=prepareScripts();function rpDebug(method='log',...args){if(window.location.search.includes('rapidload_debug_js')){console[method](...args)}}rpDebug('info','totalScripts');rpDebug('table',totalScripts);if(window.RAPIDLOAD_EXPERIMENT__DELAY_PREFETCH){document.addEventListener('DOMContentLoaded:norapidload',()=>{totalScripts.forEach((script,index)=>{setTimeout(()=>{fetch(script.scriptElement.getAttribute('data-rapidload-src')).then(()=>{script.asyncLoaded=true;script.success=true;totalScripts[index]=script}).catch(()=>{script.asyncLoaded=true;script.success=falsej;totalScripts[index]=script}).finally(()=>{onScriptAsyncLoad(script)})},10)})})}function createBatches(scripts){const scriptMap=new Map(scripts.map(script=>{return[script.id,{...script,batch:null}]}));function assignBatch(firstScriptId,scriptId,stackSet=new Set){if(firstScriptId===null){firstScriptId=scriptId}const script=scriptMap.get(scriptId);if(!script){return-1}if(script.batch!==null){return script.batch}if(stackSet.has(scriptId)){throw new Error('RapidLoad: Circular dependency detected at script: '+scriptId)}stackSet.add(scriptId);let maxBatch=0;for(const depId of script.dependencies){const depBatch=assignBatch(firstScriptId,depId,stackSet);if(depBatch===-1){continue}maxBatch=Math.max(maxBatch,depBatch)}script.batch=maxBatch+1;stackSet.delete(scriptId);return script.batch}scripts=scripts.map(script=>{const batchedScript=assignBatch(null,script.id);return scriptMap.get(script.id)});scripts.forEach(script=>{if(script.after){const afterScript=scriptMap.get(script.after);if(afterScript){script.batch=afterScript.batch}}});return scripts}function onScriptLoad(script,success=true){totalScripts=totalScripts.map(s=>s.id===script.id&&script.loaded===null?{...script,loaded:true,success:success}:s);if(totalScripts.filter(s=>s.batch===script.batch).length===totalScripts.filter(s=>s.batch===script.batch&&s.loaded).length){var batchLoadedEvent=new CustomEvent('RapidLoad:DelayedScriptBatchLoaded',{detail:{batch:script.batch},bubbles:true,cancelable:true});document.dispatchEvent(batchLoadedEvent);rpDebug('info','fired: RapidLoad:DelayedScriptBatchLoaded : '+script.batch);rpDebug('table',totalScripts.filter(s=>s.batch===script.batch))}if(totalScripts.filter(s=>s.loaded).length===totalScripts.length){window.rapidloadScripts=totalScripts;var allScriptsLoadedEvent=new CustomEvent('RapidLoad:DelayedScriptsLoaded',{bubbles:true,cancelable:true});document.dispatchEvent(allScriptsLoadedEvent);rpDebug('info','fired: RapidLoad:DelayedScriptsLoaded');rpDebug('table',totalScripts);console.warn('RapidLoad Warning: If you intend to debug JavaScript files, kindly disable the 'RapidLoad delay js' feature.')}}function prepareScripts(){var scripts=Array.from(document.querySelectorAll('[data-rapidload-src]'));var mappedScripts=scripts.map(function(script,index){var scriptId=script.getAttribute('id');var depsAttribute=script.getAttribute('data-js-deps');var src=script.getAttribute('data-rapidload-src');var afterAttribute=script.getAttribute('data-js-after');return{id:scriptId||index,scriptElement:script,dependencies:parseDependencies(depsAttribute,scripts,scriptId),loaded:null,after:afterAttribute?afterAttribute+'-js':null,success:false,src:src}});mappedScripts=mergeDuplicateObjects(mappedScripts);return createBatches(mappedScripts)}function loadScript(script){setTimeout(()=>{var scriptElement=script.scriptElement;scriptElement.addEventListener('load',()=>onScriptLoad(script));scriptElement.addEventListener('error',()=>onScriptLoad(script,false));if(script.src){scriptElement.setAttribute('type','text/javascript');scriptElement.setAttribute('src',script.src)}},35)}async function fetchAllScripts(){return Promise.all(totalScripts.map(async s=>{let response=await fetch(s.src);if(response.ok){let scriptContent=await response.text();let blob=new Blob([scriptContent],{type:'text/javascript'});s.src=URL.createObjectURL(blob)}return s}))}async function loadScriptsInDependencyOrder(){totalScripts=await fetchAllScripts();totalScripts.filter(s=>s.batch===1).forEach(function(script){loadScript(script)});document.addEventListener('RapidLoad:DelayedScriptBatchLoaded',event=>{var batch=Number(event.detail.batch)+1;if(batch>totalScripts.filter(s=>s.batch).length){return}totalScripts.filter(s=>s.batch===batch).forEach(function(script){loadScript(script)})})}function parseDependencies(depsAttribute,scriptMap,scriptId){let deps=[];let depAttributes=depsAttribute?depsAttribute.split(', '):[];deps=depAttributes.map(function(dep){var matchingScript=scriptMap.find(function(script){return script.id===dep+'-js'||dep==='jquery'&&script.id==='jquery-core-js'||dep.startsWith('jquery-ui-')&&script.id==='jquery-ui-core-js'});if(matchingScript){return matchingScript.id}else{console.warn('Dependency not found for:',dep);return null}}).filter(Boolean);if(scriptId!=='jquery-core-js'&&scriptId&&scriptId.includes('jquery-')){deps.push('jquery-core-js')}return deps}['mousemove','touchstart','keydown'].forEach(function(event){var listener=async function(){removeEventListener(event,listener);await loadScriptsInDependencyOrder();if(totalScripts===0){var allScriptsLoadedEvent=new CustomEvent('RapidLoad:DelayedScriptsLoaded',{bubbles:true,cancelable:true});document.dispatchEvent(allScriptsLoadedEvent)}};addEventListener(event,listener)})})();";
